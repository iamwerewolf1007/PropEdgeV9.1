#!/usr/bin/env python3
"""
PropEdge V9.1 — Generate Season JSONs (one-time)
==================================================
1. Generate season_2024_25.json with synthetic lines + model predictions + grades
2. Backfill season_2025_26.json with all graded historical plays
Both files are LOCKED after generation — no further edits.
"""
import pandas as pd
import numpy as np
...
from config import *
from synthetic_lines import generate_season_lines, generate_synthetic_line
...
def _clean_for_json(obj):
    """Recursively convert all numpy types to native Python for JSON serialization."""
...
def run_model_on_props(props_df, game_logs, h2h_df, model, player_trust, season_label):
    """Run the full prediction engine on a set of props. Returns list of play dicts."""
    combined = game_logs.copy()
    ...
    player_idx = {pn:g.sort_values('GAME_DATE') for pn,g in combined.groupby('PLAYER_NAME')}
    ...
    for _,prop in props_df.iterrows():
        ...
        player = prop['Player']; date_str = prop['Date'].strftime('%Y-%m-%d')
        line = prop['Line']; game = prop['Game']
        home_team = prop['Home']; away_team = prop['Away']
        raw_pos = str(prop.get('Position','') or '')
        position = POS_MAP.get(raw_pos, 'Forward')

        if player not in player_idx: skipped+=1; continue
        phist = player_idx[player]
        prior = phist[phist['GAME_DATE'] < prop['Date']]
        if len(prior) < 5: skipped+=1; continue
        snap = prior.iloc[-1]
        L30=snap.get('L30_PTS')
        if pd.isna(L30): skipped+=1; continue
        L20=snap.get('L20_PTS',L30); L10=snap.get('L10_PTS',L30)
        L5=snap.get('L5_PTS',L30); L3=snap.get('L3_PTS',L30)
        for a in ['L20','L10','L5','L3']:
            if pd.isna(eval(a)): exec(f'{a}=L30')

        team_abr = snap.get('GAME_TEAM_ABBREVIATION','')
        is_home = team_abr == home_team
        opp = away_team if is_home else home_team

        recent_pts = prior.tail(20)['PTS'].values
        if len(recent_pts) < 5: skipped+=1; continue
        recent10 = recent_pts[-10:]
        recent20 = list(recent_pts[-20:].astype(int))
        recent20_homes = list(prior.tail(20)['IS_HOME'].values.astype(int))

        vol=round(float(L30-line),1); trend=round(float(L5-L30),1)
        std10=float(np.std(recent10))
        hr10=round(sum(1 for r in recent10 if r>line)/len(recent10)*100)
        hr30=round(sum(1 for r in recent_pts if r>line)/len(recent_pts)*100)

        fg30=snap.get('L30_FG_PCT'); fg10=snap.get('L10_FG_PCT')
        min30=snap.get('L30_MIN_NUM'); min10=snap.get('L10_MIN_NUM')
        if pd.notna(fg30) and fg30<1.5: fg30*=100
        if pd.notna(fg10) and fg10<1.5: fg10*=100
        fgTrend=round(fg10-fg30,1) if pd.notna(fg10) and pd.notna(fg30) else None
        minTrend=round(min10-min30,1) if pd.notna(min10) and pd.notna(min30) else None

        defP=get_dvp(opp,position); defO=get_def_overall(opp); opp_pace=pace_rank.get(opp,15)

        hr=h2h_lkp.get((player,opp))
        h2hG=int(hr['H2H_GAMES']) if hr is not None else 0
        h2h_avg=float(hr['H2H_AVG_PTS']) if hr is not None else None
        h2h_ts_dev=float(hr['H2H_TS_VS_OVERALL']) if hr is not None and pd.notna(hr.get('H2H_TS_VS_OVERALL')) else 0
        h2h_fga_dev=float(hr['H2H_FGA_VS_OVERALL']) if hr is not None and pd.notna(hr.get('H2H_FGA_VS_OVERALL')) else 0
        h2h_min_dev=float(hr['H2H_MIN_VS_OVERALL']) if hr is not None and pd.notna(hr.get('H2H_MIN_VS_OVERALL')) else 0
        h2h_conf_v=float(hr['H2H_CONFIDENCE']) if hr is not None and pd.notna(hr.get('H2H_CONFIDENCE')) else 0
        ...
        mp=prior.tail(10)['MIN_NUM']
        min_cv=float(mp.std()/mp.mean()) if mp.mean()>0 else 1
        ...
        if model is not None:
            ...
            fd={'l30':L30,'l10':L10,'l5':L5,'l3':L3,'volume':vol,'trend':trend,'std10':std10,
                'defP':defP,'pace_rank':opp_pace,'h2h_ts_dev':h2h_ts_dev,'h2h_fga_dev':h2h_fga_dev,
                'h2h_min_dev':h2h_min_dev,'h2h_conf':h2h_conf_v,'min_cv':min_cv,
                'pts_per_min':ppm if pd.notna(ppm) else 0,'recent_min_trend':rmt if pd.notna(rmt) else 0,
                'fga_per_min':fpm if pd.notna(fpm) else 0,'is_b2b':is_b2b,'rest_days':rest,
        ...
        s={1:np.clip((L30-line)/5,-1,1),2:(hr30/100-0.5)*2,3:(hr10/100-0.5)*2,
           4:np.clip((L5-L30)/5,-1,1),5:np.clip(vol/5,-1,1),6:np.clip((defP-15)/15,-1,1),
           7:np.clip((h2h_avg-line)/5,-1,1) if use_h2h else 0.0,
           8:np.clip((15-opp_pace)/15,-1,1),9:np.clip((fgTrend or 0)/10,-1,1),
           10:np.clip((minTrend or 0)/5,-1,1)}
        if not use_h2h:
            tw=sum(w for k,w in W.items() if k!=7); ws=sum(W[k]*s[k] for k in s if k!=7)
        else:
        ...
        for nm,ag,dt in [
            ...
            ('Pace',(is_over and opp_pace<15) or (not is_over and opp_pace>15),f"#{opp_pace}"),
        ...
        # Also check prop's built-in actual if synthetic
        ...
        ro=sum(1 for r in recent20 if r>line); ru=sum(1 for r in recent20 if r<=line)

        ms=game; full_ms=game
        plays.append({
            'date':date_str,'player':player,'match':ms,'fullMatch':full_ms,
            ...
            'hr30':hr30,'hr10':hr10,
            'recent':recent20[:5],'recent10':recent20[:10],'recent20':recent20,
            'recent20homes':[bool(x) for x in recent20_homes],
            'defO':defO,'defP':defP,'pace':opp_pace,
            'h2h':h2h_str,'h2hG':h2hG,
            'h2hTsDev':_safe(h2h_ts_dev),'h2hFgaDev':_safe(h2h_fga_dev),
            'h2hConfidence':_safe(h2h_conf_v),'h2hProfile':hr.get('H2H_SCORING_PROFILE','') if hr is not None else '',
            'fgL30':_safe(fg30),'fgL10':_safe(fg10),
            'fga30':_safe(snap.get('L30_FGA')),'fga10':_safe(snap.get('L10_FGA')),
            'fg3L30':None,'fg3L10':None,
            'minL30':_safe(min30),'minL10':_safe(min10),'std10':round(std10,1),
            ...
            'preMatchReason':'','actualPts':_safe(actual),'result':result,'delta':_safe(delta),'reason':'',
...
def main():
    ...
    print(f"  2024-25: {len(plays_25)} plays, {len(graded_25)} graded, {wins_25}W/{len(graded_25)-wins_25}L = {wins_25/len(graded_25)*100:.1f}%")
    ...
    # === 2025-26: Process real prop lines ===
    ...
    print(f"  Running model on 2025-26...")
    ...
    print(f"  2025-26: {len(plays_26)} plays, {len(graded_26)} graded, {wins_26}W/{len(graded_26)-wins_26}L = {wins_26/len(graded_26)*100:.1f}%")
