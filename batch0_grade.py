...
=======================================================
Changes:
- Grade plays and write post-match reasoning via reasoning_engine.py
- Cross-check: recompute yesterday's rolling stats vs prediction, flag discrepancies
- Write Graded sheet in daily Excel
- Rolling stats for new game logs computed LIVE (not reading stale CSV columns)
"""
import pandas as pd
import numpy as np
...
MASTER_DIR = Path(__file__).parent.resolve() / 'master'
    ...
    try:
        ...
        if r2.returncode != 0:
            ...
            if 'nothing to commit' in msg:
                print(f"  ✓ Git: nothing to commit"); return
...
def fetch_boxscores(date_str):
    ...
    for g in gids:
        ...
        if len(r) >= 2:
            ctx[str(g)] = {
                'htid': r.iloc[0]['teamId'], 'ht': r.iloc[0]['teamTricode'],
                'at': r.iloc[1]['teamTricode'],
                'hs': _si(r.iloc[0].get('score', 0)), 'as': _si(r.iloc[1].get('score', 0)),
            }
    ...
    for g in gids:
        ...
        try:
            ...
            for _, p in ps.iterrows():
                ...
                opp = ps[ps['TEAM_ID'] != tid]['TEAM_ABBREVIATION'].iloc[0] \
                      if len(ps[ps['TEAM_ID'] != tid]) > 0 else 'UNK'
                mu  = f"{ta} vs. {opp}" if ih else f"{ta} @ {opp}"
                wl  = ('W' if c.get('hs', 0) > c.get('as', 0) else 'L') if ih \
                      else ('W' if c.get('as', 0) > c.get('hs', 0) else 'L')
                ...
                fgp = fgm/fga if fga>0 else 0; f3p=fg3m/fg3a if fg3a>0 else 0
                ftp = ftm/fta if fta>0 else 0
                efg = (fgm+0.5*fg3m)/fga if fga>0 else 0
                tsa = 2*(fga+0.44*fta); ts=pts/tsa if tsa>0 else 0
                usg = (fga+0.44*fta+tov)/(mn/5) if mn>0 else 0
                pra = pts+reb+ast
                ...
                rows.append({
                    'PLAYER_ID': pid,
                    'PLAYER_NAME': p.get('PLAYER_NAME', b.get('PLAYER_NAME', '')),
...
def append_gamelogs(new_rows):
    """
    Append new game rows to the 2025-26 CSV.
    Rolling L* columns are computed LIVE for the new rows only.
    ...
    # Initialise all rolling columns as NaN on new rows
    for w in WINDOWS:
        for c in ROLL_COLS:
            ndf[f'L{w}_{c}'] = np.nan

    # Combined history = existing data (no new rows yet)
    ...
    # LIVE rolling computation for each new row
    # Never groupby().apply() — explicit loops
    for pid in ndf['PLAYER_ID'].unique():
        ...
        for i, (idx, r) in enumerate(pn.iterrows()):
            # Prior = all historical + new rows before this one
            if i > 0:
                prior = pd.concat([ph, pn.iloc[:i]]).sort_values('GAME_DATE')
            else:
                prior = ph.copy()
            ...
            for w in WINDOWS:
                wd = prior.tail(w)
                for c in ROLL_COLS:
                    if len(wd) == 0 or (len(wd) < w // 2 and w >= 100):
                        ndf.at[idx, f'L{w}_{c}'] = np.nan
                    elif c in wd.columns:
                        v = wd[c].mean()
                        ndf.at[idx, f'L{w}_{c}'] = round(float(v), 4) if pd.notna(v) else np.nan
    ...
    b4  = len(upd)
    upd = upd.drop_duplicates(subset=['PLAYER_ID', 'GAME_ID'], keep='last')
...
def grade_plays(date_str, new_rows):
    ...
    Adds post-match reasoning and loss classification.
    """
    results_map = {(r['PLAYER_NAME'], date_str): r['PTS'] for r in new_rows}
    graded_count = wins = losses = dnp = 0
    ...
    for fpath in [SEASON_2526, TODAY_JSON]:
        ...
        for p in plays:
            ...
            if lkp is None:
                p['result'] = 'DNP'; p['actualPts'] = None; dnp += 1; changed = True; continue

            actual = int(lkp); p['actualPts'] = actual
            ...
            try:
                post_reason, loss_type = generate_post_match_reason(p)
                p['postMatchReason'] = post_reason
...
def update_daily_excel_graded(date_str, results_map):
    """
    Update the Graded sheet in daily/YYYY-MM-DD.xlsx with actual results.
    """
    excel_path = DAILY_DIR / f"{date_str}.xlsx"
    if not excel_path.exists():
        print(f"  ⚠ No daily Excel found for {date_str}")
        return
    ...
    try:
        ...
        if 'Graded' not in wb.sheetnames:
            print("  ⚠ No Graded sheet in daily Excel"); return

        ws = wb['Graded']
        now_str = now_uk().strftime('%Y-%m-%d %H:%M')
        green = PatternFill(start_color="22C55E", end_color="22C55E", fill_type="solid")
        red   = PatternFill(start_color="EF4444", end_color="EF4444", fill_type="solid")

        # Headers: Player(1), Match(2), Dir(3), Line(4), ActualPts(5), Result(6),
        #          Delta(7), PostMatchReason(8), LossType(9), GradedAt(10)
        for row in ws.iter_rows(min_row=2):
            player = row[0].value; match_val = row[1].value
            dir_val = row[2].value; line_val = row[3].value
            if not player or not line_val: continue

            # Look up from JSON for post-match reason
            actual = None
            result = None
            delta  = None
            post_reason = ''
            ...
            for fpath in [TODAY_JSON, SEASON_2526]:
                if not fpath.exists(): continue
                with open(fpath) as f: plays_data = json.load(f)
                for p in plays_data:
                    if p.get('player') == player and p.get('date') == date_str:
                        actual = p.get('actualPts')
                        result = p.get('result')
                        delta  = p.get('delta')
                        post_reason = p.get('postMatchReason', '')
                        loss_type = p.get('lossType', '')
                        break
                if result: break
            ...
            if result:
                ...
                row[5].value = result
                row[6].value = delta
                row[7].value = post_reason
                row[8].value = loss_type or ''
                row[9].value = now_str
                if result == 'WIN':
...
def crosscheck_rolling_stats(date_str):
    """
    Cross-check: recompute rolling stats for yesterday's plays using today's
    master CSV and compare against what was used in predictions.
    Logs discrepancies > 1 pt to audit_log.csv.
    """
    excel_path = DAILY_DIR / f"{date_str}.xlsx"
    ...
    try:
        ...
        import openpyxl
        wb = openpyxl.load_workbook(excel_path)
        if 'Props' not in wb.sheetnames: return
        ws = wb['Props']

        discrepancies = []
        for row in ws.iter_rows(min_row=2, values_only=True):
            player, match, pos, line = row[0], row[1], row[2], row[3]
            old_l30 = row[8]  # L30 column index in Props sheet
            if not player or not line or not old_l30: continue
            ...
            diff = abs(new_l30 - float(old_l30)) if old_l30 else 0
            if diff > 1.0:
                discrepancies.append({
                    'player': player, 'date': date_str,
                    'old_l30': old_l30, 'new_l30': new_l30, 'diff': round(diff, 1)
                })

        if discrepancies:
            print(f"  ⚠ Cross-check: {len(discrepancies)} rolling stat discrepancies (>1pt)")
            for d in discrepancies[:5]:
                print(f"    {d['player']}: old={d['old_l30']} new={d['new_l30']} diff={d['diff']}")
            log_event('B0', 'CROSSCHECK_DISCREPANCIES',
                      detail=f"{len(discrepancies)} diffs on {date_str}")
        else:
            print(f"  ✓ Cross-check: all rolling stats consistent for {date_str}")
            log_event('B0', 'CROSSCHECK_PASSED', detail=date_str)
...
def main():
    ...
    results_map = grade_plays(yesterday, rows)
    update_daily_excel_graded(yesterday, results_map)
    crosscheck_rolling_stats(yesterday)
